...
std::vector<HailoDetection> yolov5_decoding(xt::xarray<float> &output, const int stride, xt::xarray<float> &anchors, xt::xarray<float> &grid, xt::xarray<float> &anchor_grid, const int num_anchors, const float score_threshold)
{
    ...
    auto all_scores = xt::view(all_decoded, xt::all(), xt::range(5, 5+num_classes));
    ...
    auto masks = xt::view(all_decoded, xt::keep(indices), xt::range(5+num_classes, _));
...
std::vector<HailoDetection> yolov5seg_post(auto &tensors, auto &anchor_list, auto &stride_list, const float iou_threshold, const float score_threshold, auto &grids, auto &anchor_grids, const int num_anchors)
{
    ...
    // std::cout<< "test" << tensors["yolov5m_seg/conv85"]->vstream_info().shape.width << std::endl;
    auto proto_tensor = common::dequantize(common::get_xtensor(tensors["yolov5m_seg/conv85"]), tensors["yolov5m_seg/conv85"]->vstream_info().quant_info.qp_scale, tensors["yolov5m_seg/conv85"]->vstream_info().quant_info.qp_zp);
    // auto proto_tensor = common::dequantize(common::get_xtensor(tensors["yolov5m_seg_1class/conv85"]), tensors["yolov5m_seg_1class/conv85"]->vstream_info().quant_info.qp_scale, tensors["yolov5m_seg_1class/conv85"]->vstream_info().quant_info.qp_zp);
    // run the postprocess for each branch seperately
    // std::future<std::vector<HailoDetection>> t2 = std::async(post_per_branch, "yolov5m_seg_1class/conv66", 2, tensors, anchor_list, stride_list, iou_threshold, score_threshold, grids, anchor_grids, num_anchors);
    // std::future<std::vector<HailoDetection>> t1 = std::async(post_per_branch, "yolov5m_seg_1class/conv75", 1, tensors, anchor_list, stride_list, iou_threshold, score_threshold, grids, anchor_grids, num_anchors);
    // std::future<std::vector<HailoDetection>> t0 = std::async(post_per_branch, "yolov5m_seg_1class/conv83", 0, tensors, anchor_list, stride_list, iou_threshold, score_threshold, grids, anchor_grids, num_anchors);
    std::future<std::vector<HailoDetection>> t2 = std::async(post_per_branch, "yolov5m_seg/conv66", 2, tensors, anchor_list, stride_list, iou_threshold, score_threshold, grids, anchor_grids, num_anchors);
    std::future<std::vector<HailoDetection>> t1 = std::async(post_per_branch, "yolov5m_seg/conv75", 1, tensors, anchor_list, stride_list, iou_threshold, score_threshold, grids, anchor_grids, num_anchors);
    std::future<std::vector<HailoDetection>> t0 = std::async(post_per_branch, "yolov5m_seg/conv83", 0, tensors, anchor_list, stride_list, iou_threshold, score_threshold, grids, anchor_grids, num_anchors);
